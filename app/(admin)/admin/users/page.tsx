                ...
                <div className="flex-1 flex flex-col gap-6 max-w-7xl mx-auto w-full overflow-hidden">
                    ...
                    <div className="flex items-center justify-between lg:block">
                        <div>
                            ...
                            <p className="text-xs md:text-sm text-gray-500 mt-1">مشاهده مشخصات، مدیریت نقش‌ها و تعیین وضعیت دسترسی کاربران سایت</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center gap-4 shadow-sm">
                            ...
                            <div>
                                <span className="block text-xs md:text-sm text-gray-400 font-medium">کل کاربران</span>
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center gap-4 shadow-sm">
                            ...
                            <div>
                                <span className="block text-xs md:text-sm text-gray-400 font-medium">کاربران فعال</span>
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center gap-4 shadow-sm sm:col-span-2 lg:col-span-1">
                            ...
                            <div>
                                <span className="block text-xs md:text-sm text-gray-400 font-medium">حساب‌های مسدود</span>
                    ...
                    <div className="bg-white rounded-2xl p-4 md:p-5 border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
                        <div className="w-full md:max-w-md relative">
                            <input 
                                ...
                                className="w-full font-medium bg-gray-50 border border-gray-200/80 rounded-xl pr-11 pl-4 py-3 outline-none transition-all focus:border-[#D92F4E] focus:bg-white focus:ring-4 focus:ring-[#D92F4E]/5 text-sm text-gray-800 placeholder-gray-400"
                            />
                            ...
                            <table className="w-full text-right border-collapse min-w-[800px]">
                                <thead>
                                    <tr className="bg-gray-50/70 text-gray-500 font-bold text-xs uppercase tracking-wider border-b border-gray-100">
                                        <th className="p-4 min-w-[160px]">نام و نام خانوادگی</th>
                                ...
                                <tbody className="divide-y divide-gray-100 font-medium text-sm text-gray-600">
                                    {usersSample.map((user) => (
                                        <tr key={user.id} className="hover:bg-gray-50/40 transition-colors group">
                                            ...
                                            <td className="p-4 font-mono text-xs text-gray-600 tracking-wider" dir="ltr">{user.contact}</td>
                                            <td className="p-4 text-gray-500 font-mono text-xs">{user.email}</td>
                                            <td className="p-4 text-center">
                                                <span className={`inline-block px-2.5 py-1 rounded-lg text-xs font-bold ${user.roleStyle}`}>
                                                    {user.role}
                                                </span>
                                            </td>
                                            <td className="p-4 text-center text-gray-400 text-xs font-semibold">{user.date}</td>
                                            <td className="p-4 text-center">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${user.statusStyle}`}>
                                                    <span className={`w-1.5 h-1.5 rounded-full ${user.status === 'فعال' ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
                                                    {user.status}
                                                </span>
                                            </td>
                                            <td className="p-4 text-center">
