    ...
    return (
        <>
            ...
            `}>
                ...
                {/* پروفایل کاربر */}
                <div className="flex flex-col items-center text-center gap-2 pb-5 border-b border-slate-100 mt-4 lg:mt-0">
                    <div className="relative w-20 h-20 rounded-full p-1 border-2 border-[#D92F4E]/30 shadow-md shadow-[#D92F4E]/10">
                        <Image src='/images/admin.jpg' fill alt="تصویر ادمین" className="object-cover rounded-full" />
                    </div>
                    <div>
                        <h2 className="font-bold text-slate-900  md:">میثاق باباخانی</h2>
                        <span className="font-medium text-slate-400 block mt-0.5  md:">مدیر کل مجموعه</span>
                    </div>
                </div>
                            ...
                            return (
                                <li key={index}>
                                    <Link 
                                        href={item.href} 
                                        onClick={onClose}
                                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-bold  md: transition-all duration-200 group ${
                                            isActive 
                                            ? 'bg-[#D92F4E] text-white shadow-lg shadow-[#D92F4E]/25' 
                                            : 'text-slate-700 hover:bg-slate-50 hover:text-slate-900'
                                        }`}
                                    >
                                        <svg className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-900'}`}>
                ...
                <div className="pt-4 mt-2 border-t border-slate-100">
                    <button
                        ...
                        disabled={loggingOut}
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-2xl font-bold  md: text-rose-600 bg-rose-50/80 hover:bg-rose-100/80 active:scale-[0.98] transition-all duration-200 disabled:opacity-60 cursor-pointer"
                    >
